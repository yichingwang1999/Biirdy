...
embed_size = 256
hidden_size = 256
vocab_size = 2994  # 这里需要你有访问词汇表的方式
num_layers = 1
...
checkpoint = torch.load(
    "/Users/olliewang/Desktop/final project/models/my_checkpoint.pth.tar", map_location=torch.device('cpu'))
...
def generate_caption():
    ...
    # 将图片转为你的模型需要的格式
    # 例如，你可能需要转换为 Tensor 等
    image = process_image(file)

    # 使用模型生成描述
    with torch.no_grad():
        description = model(image)  # 请根据您的模型进行相应的调整
